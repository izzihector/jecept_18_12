...
class BillReport(models.TransientModel):
    _name = "bill.report"
    ...
    sort_by = fields.Selection([('name', 'Bill Ref'),
                               ('date', 'Bill Date'),
    ...
    target = fields.Selection([('in_invoice', 'Only Vendor Bills'),
                               ('in_refund', 'Only Refunds'),
                               ('both', 'Both Vendor Bills & Refunds'),
                               ], string='Target Moves', required=True, default='in_invoice')
    ...
    def action_print(self):
        ...
        return self.env.ref('invoices_report.action_bill_report_pdf').report_action(self, data=report_lines)
    ...
    def get_report_lines(self, data):
        ...
        if target == 'in_invoice':
            domain += [('move_type', '=', 'in_invoice')]
        elif target == 'in_refund':
        ...
        in_invoices = move_ids.filtered(lambda c: c.move_type == 'in_invoice')
        in_refunds = move_ids.filtered(lambda c: c.move_type == 'in_refund')

        # getting customer invoices
        if target in ['in_invoice', 'both']:
            ...
            report_lines['bills'] = bills
            report_lines['bills_totals'] = {
            ...
            tax_summary = self.get_tax_summary(in_invoices.ids)
            report_lines['bills_tax_summary'] = tax_summary

        # getting credit notes
        if target in ['in_refund', 'both']:
            sum_amt_untaxed = 0.00
            sum_amt_tax = 0.00
            sum_amt_total = 0.00
            sum_amt_discount = 0.00
            ...
            tax_summary = self.get_tax_summary(in_refunds.ids)
            report_lines['refunds_tax_summary'] = tax_summary

        if target == 'both':
            report_lines['net_totals'] = {
                'net_amt_untaxed': round(report_lines['bills_totals']['sum_amt_untaxed'] + report_lines['refunds_totals']['sum_amt_untaxed'], 3),
                'net_amt_tax': round(report_lines['bills_totals']['sum_amt_tax'] + report_lines['refunds_totals']['sum_amt_tax'], 3),
                'net_amt_total': round(report_lines['bills_totals']['sum_amt_total'] + report_lines['refunds_totals']['sum_amt_total'], 3),
                'net_amt_discount': round(report_lines['bills_totals']['sum_amt_discount'] + report_lines['refunds_totals']['sum_amt_discount'], 3),
                # 'net_amt_cost': round(report_lines['bills_totals']['sum_amt_cost'] + report_lines['refunds_totals']['sum_amt_cost'], 3),
                # 'net_amt_profit': round(report_lines['bills_totals']['sum_amt_profit'] + report_lines['refunds_totals']['sum_amt_profit'], 3),
            }
