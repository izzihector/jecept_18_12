...
class NetProductMovementWizard(models.TransientModel):
    _name = "net.product.movement.wizard"
    _description = 'Net Product Movement Report'

    product_id = fields.Many2one('product.product', 'Product', required=True)
    location_id = fields.Many2one('stock.location', string='Location')
    from_date = fields.Date("From Date", default=datetime.now(), required=True)
    to_date = fields.Date("To Date", default=datetime.now(), required=True)
    ...
    def get_sale_line(self):
        ...
        domain += ['|', ('picking_code', '=', 'outgoing'), ('location_id', '=', self.env['stock.location'].search([('name', '=', 'Stock')]).id),('state', '=', 'done')]
    ...
    def get_purchase_line(self):
        ...
        domain += ['|', ('picking_code', '=', 'incoming'), ('location_dest_id', '=', self.env['stock.location'].search([('name', '=', 'Stock')]).id), ('state', '=', 'done')]
        stock_move_line = self.env["stock.move.line"].search(domain)
        product_grouped = []
        for line in stock_move_line:
    ...
    def get_opening_balance(self):
        ...
        purchase_domain += ['|', ('picking_code', '=', 'incoming'), ('location_dest_id', '=', self.env['stock.location'].search([('name', '=', 'Stock')]).id), ('state', '=', 'done')]
        ...
        sale_domain += ['|', ('picking_code', '=', 'outgoing'), ('location_id', '=', self.env['stock.location'].search([('name', '=', 'Stock')]).id), ('state', '=', 'done')]
    ...
    def print_report(self):
        ...
        return self.env.ref('net_product_movement_report.action_net_product_movement_report').report_action(self, data=data)
    ...
    #         domain += [('product_id', '=', self.product_id.id)]
    #
    #     domain += [('picking_code', '=', 'outgoing'), ('state', '=', 'done')]
    #     stock_move_line = self.env["stock.move.line"].search(domain)
    #     product_grouped = {}
    #     for line in stock_move_line:
    #         if line.move_id.sale_line_id:
    #             if line.product_id.id not in product_grouped.keys():
    #                 product_grouped[line.product_id.id] = {'name': 'sale',
    #                                                        'qty': line.qty_done,
    #                                                        'amount': line.move_id.sale_line_id.price_subtotal
    #                                                        }
    #             elif line.product_id.id in product_grouped.keys():
    #                 product_grouped[line.product_id.id]['qty'] += line.qty_done
    #                 product_grouped[line.product_id.id]['amount'] += line.move_id.sale_line_id.price_subtotal
    #
    #         else:
    #             if line.product_id.id not in product_grouped.keys():
    #                 product_grouped[line.product_id.id] = {'name': 'outgoing',
    #                                                        'qty': line.qty_done,
    #                                                        'amount': line.qty_done * line.product_id.standard_price
    #                                                        }
    #             elif line.product_id.id in product_grouped.keys():
    #                 product_grouped[line.product_id.id]['qty'] += line.qty_done
    #                 product_grouped[line.product_id.id]['amount'] += line.qty_done * line.product_id.standard_price
    #
    #
    #     return product_grouped
    #
    ...
    #     # for line in all_order_lines:
    #     #     if line.product_id.id not in product_grouped.keys():
    #     #         product_grouped[line.product_id.id] = {'name': 'فاتورة بيع',
    #     #                                                'qty': line.product_uom_qty,
    #     #                                                'amount': line.price_subtotal,
    #     #                                                }
    #     #     elif line.product_id.id in product_grouped.keys():
    ...
    #         if line.move_id.purchase_line_id:
    #             if line.product_id.id not in product_grouped.keys():
    #                 product_grouped[line.product_id.id] = {'name': 'purchase',
    #                                                        'qty': line.qty_done,
    #                                                        'amount': line.move_id.purchase_line_id.price_subtotal
    #                                                        }
    #             elif line.product_id.id in product_grouped.keys():
    #                 product_grouped[line.product_id.id]['qty'] += line.qty_done
    #                 product_grouped[line.product_id.id]['amount'] += line.move_id.purchase_line_id.price_subtotal
    #
    #         else:
    #             if line.product_id.id not in product_grouped.keys():
    #                 product_grouped[line.product_id.id] = {'name': 'incoming',
    #                                                        'qty': line.qty_done,
    #                                                        'amount': line.qty_done * line.product_id.standard_price
    #                                                        }
    #             elif line.product_id.id in product_grouped.keys():
    ...
    #     # for line in all_order_lines:
    #     #     if line.product_id.id not in product_grouped.keys():
    #     #         product_grouped[line.product_id.id] = {'name': 'فاتورة شراء',
    #     #                                                'qty': line.product_uom_qty,
    #     #                                                'amount': line.price_subtotal,
    #     #                                                }
    #     #     elif line.product_id.id in product_grouped.keys():
    ...
    #         purchase_domain += [('date', '<', self.from_date)]
    #     if self.location_id:
    #         purchase_domain += ['|', ('location_id', '=', self.location_id.id), ('location_dest_id', '=', self.location_id.id)]
    #     if self.product_id:
    #         purchase_domain += [('product_id', '=', self.product_id.id)]
    #
    #     purchase_domain += [('picking_code', '=', 'incoming'), ('state', '=', 'done')]
    #     stock_move_line = self.env["stock.move.line"].search(purchase_domain)
    #     purchase_product_grouped = {}
    #     for line in stock_move_line:
    #         if line.product_id.id not in purchase_product_grouped.keys():
    ...
    #         sale_domain += [('date', '<', self.from_date)]
    #     if self.location_id:
    #         sale_domain += ['|', ('location_id', '=', self.location_id.id), ('location_dest_id', '=', self.location_id.id)]
    #     if self.product_id:
    #         sale_domain += [('product_id', '=', self.product_id.id)]
    #
    #     sale_domain += [('picking_code', '=', 'outgoing'), ('state', '=', 'done')]
    #     stock_move_line = self.env["stock.move.line"].search(sale_domain)
    #     sale_product_grouped = {}
    #     for line in stock_move_line:
    #         if line.product_id.id not in sale_product_grouped.keys():
    ...
    #     # purchase_order = self.env["purchase.order"].search(purchase_domain)
    #     # purchase_product_grouped = {}
    #     # all_purchase_order_lines = purchase_order.mapped('order_line').filtered(lambda r: r.product_id == self.product_id)
    #     #
    #     # for line in all_purchase_order_lines:
    #     #     if line.product_id.id not in purchase_product_grouped.keys():
    #     #         purchase_product_grouped[line.product_id.id] = {'name': 'فاتورة شراء',
    #     #                                                'qty': line.product_uom_qty,
    #     #                                                'amount': line.price_subtotal,
    #     #                                                }
    #     #     elif line.product_id.id in purchase_product_grouped.keys():
    ...
    #     # for line in all_sale_order_lines:
    #     #     if line.product_id.id not in sale_product_grouped.keys():
    #     #         sale_product_grouped[line.product_id.id] = {'name': 'فاتورة بيع',
    #     #                                                'qty': line.product_uom_qty,
    #     #                                                'amount': line.price_subtotal,
    #     #                                                }
    #     #     elif line.product_id.id in sale_product_grouped.keys():
    ...
    #
    #
    #     for new_s, new_val in purchases_product.items():
    #         # print first key
    #         purchases_product = purchases_product[new_s]
    #         # after getting first key break loop
    #         break
    #
    #     for new_s, new_val in sales_product.items():
    #         # print first key
    #         sales_product = sales_product[new_s]
    #         # after getting first key break loop
    #         break
    #
    #     if not sales_product:
    #         sales_product = {'name': 'outgoing', 'qty': 0.000, 'amount': 0.000,}
    #     if not purchases_product:
    #         purchases_product = {'name': 'incoming', 'qty': 0.000, 'amount': 0.000,}
    #
    #     opening_balance_qty = self.get_opening_balance()['opening_balance_qty']
    #     opening_balance_amount = self.get_opening_balance()['opening_balance_amount']
    #
